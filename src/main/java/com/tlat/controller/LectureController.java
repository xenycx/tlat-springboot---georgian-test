...
public class LectureController {
    ...
    public String showAddForm(Model model) {
        ...
        model.addAttribute("existingLectures", lectureService.findAllLectures());
    ...
    @PreAuthorize("hasAnyRole('ADMIN','LECTURER')")
    @PostMapping("/add-schedule-only")
    public String addScheduleOnly(@ModelAttribute("lecture") LectureDto lecture,
                                  RedirectAttributes redirectAttributes) {
        try {
            if (lecture.getLectureId() == null) {
                throw new RuntimeException("ლექცია არ არის არჩეული");
            }
            lectureService.addSchedule(lecture.getLectureId(), lecture);
            redirectAttributes.addFlashAttribute("successMessage", "განრიგი წარმატებით დაემატა არსებულ კურსს");
        } catch (Exception e) {
    ...
    public String deleteLecture(@PathVariable Long id, Principal principal, RedirectAttributes redirectAttributes) {
        try {
            User user = userService.findUserByEmail(principal.getName());
            validateLecturePermission(id, user);
            lectureService.deleteLectureById(id);
            redirectAttributes.addFlashAttribute("successMessage", "კურსი წარმატებით წაიშალა");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errorMessage", "ვერ მოხერხდა კურსის წაშლა: " + e.getMessage());
        }
        return "redirect:/lectures";
    }
